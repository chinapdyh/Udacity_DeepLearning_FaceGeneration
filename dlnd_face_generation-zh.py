...
def discriminator(images, reuse=False, alpha=0.2):
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        x1 = tf.layers.conv2d(images, input_size*2, 5, strides=2, padding='same',kernel_initializer=
init)
        relu1 = tf.maximum(alpha * x1, x1)
        relu1 = tf.nn.dropout(relu1, keep_prob=0.2)

        x2 = tf.layers.conv2d(relu1, input_size*4, 5, strides=2, padding='same')
        bn2 = tf.layers.batch_normalization(x2, training=True)
        relu2 = tf.maximum(alpha * bn2, bn2)
        relu2 = tf.nn.dropout(relu2, keep_prob=0.25)

        x3 = tf.layers.conv2d(relu2, input_size*8, 5, strides=2, padding='same')
        bn3 = tf.layers.batch_normalization(x3, training=True)
        relu3 = tf.maximum(alpha * bn3, bn3)
        relu3 = tf.nn.dropout(relu3, keep_prob=0.3)

        # Flatten it
        flat = tf.reshape(relu3, (-1, 4*4*input_size*8))
    ...
    with tf.Session() as sess:
        ...
        for epoch_i in range(epoch_count):
            for batch_images in get_batches(batch_size):
                ...
                if steps % 100 == 0:
                    show_generator_output(sess, 4, net.input_z, data_shape[3], data_image_mode)
